...
export type DetectionRuleSummary = { id: string; name: string; description: string; severity: Severity }
export type DetectionRule = DetectionRuleSummary & { enabled: boolean; rule_type: string; definition: Record<string, unknown>; updated_at: string }
export type Playbook = { id: string; name: string; description: string; actions: RemediationType[]; required_role: AppRole; enabled: boolean }
export type IncidentWithEvents = Incident & { incident_events: { event: SecurityEvent }[]; detection_matches: { id: string; evidence: Record<string, unknown>; rule: DetectionRuleSummary | null }[] }
